(() => {
  ...
  const isMobileConfig = canvasEl.getAttribute('mobile') === 'false'
  const isMobile = /Android|webOS|iPhone|iPod|iPad|BlackBerry/i.test(navigator.userAgent)

  if (isMobileConfig && isMobile) return
  ...
  const setCanvasSize = btf.debounce(() => {
  ...
  const updateCoords = e => {
    ...
    pointerX = (e.clientX || e.touches[0].clientX) - rect.left
    pointerY = (e.clientY || e.touches[0].clientY) - rect.top
  }
